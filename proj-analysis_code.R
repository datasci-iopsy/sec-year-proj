...
shhh(tidyverse)
shhh(psych) #masks ggplot::alpha

#read in file (skip the second line)
all_content = readLines("./data/proj-data_cln.csv")
skip_second = all_content[-2]
dat = read.csv(textConnection(skip_second), header = TRUE, 
               stringsAsFactors = TRUE)
rm(all_content, skip_second)

#rm attention check and other unnecessary columns
dat = dat %>%
    mutate(
        Att_Chk1 = NULL,
        Att_Chk2 = NULL,
